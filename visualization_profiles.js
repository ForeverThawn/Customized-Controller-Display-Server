...
class DrawCollectionXBoxOne360Controller {
    ...
    static drawController(mapping, ctx, x, y, gamepad, options = {}) {
        x -= this.sizeController.width / 2 + 100
        ...
        this.drawSpecializedTriggerPedal(ctx,
            x + (this.sizeCase.width / 5 * 1) + this.sizeTrigger.width / 2,
            y + this.sizeTrigger.height / 2,
            mapping === "FIREFOX" ? ((gamepad.axes[2] + 1) / 2) : gamepad.buttons[6].value, options)

        this.drawSpecializedAxisPedal(ctx,
            x + (this.sizeCase.width / 5 * 3) + this.sizeTrigger.width / 2,
            y + this.sizeTrigger.height / 2,
            gamepad.axes[startIndexAxisRight + 1] < 0 ? 
            -gamepad.axes[startIndexAxisRight + 1] : 0)

        this.drawSpecializedTriggerPedal(ctx,
            x + (this.sizeCase.width / 5 * 5) - this.sizeTrigger.width / 2,
            y + this.sizeTrigger.height / 2,
            mapping === "FIREFOX" ? ((gamepad.axes[5] + 1) / 2) : gamepad.buttons[7].value, options)
    ...
    static drawTrigger(ctx, x, y, pressedValue = 0, options = {}) {
        ...
        // ctx.fillRect(x - this.infoTrigger.width / 2, 
        //     y - (this.infoTrigger.pressDepth + this.infoTrigger.baseDepth) / 2 + (this.infoTrigger.pressDepth * pressedValue), 
        //     this.infoTrigger.width, 
        //     this.infoTrigger.baseDepth + (this.infoTrigger.pressDepth * (1 - pressedValue)))

        let baseDepth = this.infoTrigger.baseDepth    // 25
        let pressDepth = this.infoTrigger.pressDepth * 5 // 35 * 3
        ctx.fillRect(x - this.infoTrigger.width / 2, 
            y + (pressDepth * pressedValue), 
            this.infoTrigger.width * 3, 
            baseDepth + (pressDepth * (1 - pressedValue)))
    ...
    static drawSpecializedTriggerPedal(ctx, x, y, pressedValue = 0, options = {}) {
        // 花活儿 绿 -> 红
        const hue = (1 - pressedValue) * 120;  // 70  // 120 // 90  // 80  // 150
        ctx.fillStyle = `hsl(${hue}, 100%, 40%)`;  // 100/40 // 100/50 淡
        ...
        let baseDepth = this.infoTrigger.baseDepth    // 25
        let pressDepth = this.infoTrigger.pressDepth * 8 // 275 //280 // 245  // 175  // 105  // 35 * 3  
        ctx.fillRect(x - this.infoTrigger.width / 2, 
            y + (pressDepth * pressedValue), 
            this.infoTrigger.width * 4, 
            baseDepth + (pressDepth * (1 - pressedValue)))
        
        ctx.font = '60px Arial';  // 50
        ctx.fillStyle = 'white';
        ctx.fillText((pressedValue * 100).toFixed(1) + '%', 
            x - 25, // 10  // 5
            y + baseDepth + pressDepth + 50)  // +20  // +10
            
        // drawDebugBoundingBox(ctx, x, y, this.sizeTrigger)
    }

    static drawSpecializedAxisPedal(ctx, x, y, axisYValue) {
        // 花活儿 绿 -> 红
        const hue = (1 - Math.sqrt(axisYValue)) * 120;  
        ctx.fillStyle = `hsl(${hue}, 100%, 40%)`;

        let baseDepth = this.infoTrigger.baseDepth    // 25
        let pressDepth = this.infoTrigger.pressDepth * 8 // 35 * 3
        ctx.fillRect(x - this.infoTrigger.width / 2, 
            y + (pressDepth * axisYValue), 
            this.infoTrigger.width * 5, 
            baseDepth + (pressDepth * (1 - axisYValue)))

        ctx.font = '60px Arial';
        ctx.fillStyle = 'white';
        ctx.fillText((axisYValue * 100).toFixed(1) + '%', 
            x - 25, 
            y + baseDepth + pressDepth + 50)
